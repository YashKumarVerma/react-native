...
RCT_ENUM_CONVERTER(
    ...
    (@{
      ...
      @"savedphotos" : @(PHAssetCollectionSubtypeAny), // This was ALAssetsGroupSavedPhotos, seems to have no direct
                                                       // correspondence in PHAssetCollectionSubtype
    }),
...
+ (PHFetchOptions *)PHFetchOptionsFromMediaType:(NSString *)mediaType {
  ...
  if ([lowercase isEqualToString:@"photos"]) {
    ...
    if (![lowercase isEqualToString:@"all"]) {
      RCTLogError(
          @"Invalid filter option: '%@'. Expected one of 'photos',"
           "'videos' or 'all'.",
          mediaType);
    }
...
RCT_EXPORT_METHOD(saveToCameraRoll
                  : (NSURLRequest *)request type
                  : (NSString *)type resolve
                  ...
                  : (RCTPromiseRejectBlock)reject)
{
  ...
  void (^loadBlock)(void) = ^void() {
    if ([type isEqualToString:@"video"]) {
      ...
      [[self.bridge moduleForClass:[RCTImageLoader class]] loadImageWithURLRequest:request
...
static void
RCTResolvePromise(RCTPromiseResolveBlock resolve, NSArray<NSDictionary<NSString *, id> *> *assets, BOOL hasNextPage)
{
  if (!assets.count) {
    resolve(@{
      @"edges" : assets,
...
RCT_EXPORT_METHOD(getPhotos
                  : (JS::NativeCameraRollManager::GetPhotosParams &)params resolve
                  : (RCTPromiseResolveBlock)resolve reject
                  : (RCTPromiseRejectBlock)reject)
{
  ...
  PHAssetCollectionType const collectionType =
      ([groupTypes isEqualToString:@"all"] ? PHAssetCollectionTypeSmartAlbum : PHAssetCollectionTypeAlbum);
  ...
  if (groupName != nil) {
    collectionFetchOptions.predicate =
        [NSPredicate predicateWithFormat:[NSString stringWithFormat:@"localizedTitle == '%@'", groupName]];
  }

  requestPhotoLibraryAccess(reject, ^{
    PHFetchResult<PHAssetCollection *> *const assetCollectionFetchResult =
        [PHAssetCollection fetchAssetCollectionsWithType:collectionType
                                                 subtype:collectionSubtype
                                                 options:collectionFetchOptions];
    [assetCollectionFetchResult
        enumerateObjectsUsingBlock:^(
            PHAssetCollection *_Nonnull assetCollection, NSUInteger collectionIdx, BOOL *_Nonnull stopCollections) {
          // Enumerate assets within the collection
          PHFetchResult<PHAsset *> *const assetsFetchResult = [PHAsset fetchAssetsInAssetCollection:assetCollection
                                                                                            options:assetFetchOptions];

          [assetsFetchResult
              enumerateObjectsUsingBlock:^(PHAsset *_Nonnull asset, NSUInteger assetIdx, BOOL *_Nonnull stopAssets) {
                ...
                if ([mimeTypes count] > 0) {
                  ...
                  NSString *const mimeType =
                      (NSString *)CFBridgingRelease(UTTypeCopyPreferredTagWithClass(uti, kUTTagClassMIMEType));

                  BOOL __block mimeTypeFound = NO;
                  [mimeTypes enumerateObjectsUsingBlock:^(
                                 NSString *_Nonnull mimeTypeFilter, NSUInteger idx, BOOL *_Nonnull stop) {
                ...
                NSString *const assetMediaTypeLabel =
                    (asset.mediaType == PHAssetMediaTypeVideo
                         ? @"video"
                         : (asset.mediaType == PHAssetMediaTypeImage
                                ? @"image"
                                : (asset.mediaType == PHAssetMediaTypeAudio ? @"audio" : @"unknown")));
...
RCT_EXPORT_METHOD(deletePhotos
                  : (NSArray<NSString *> *)assets resolve
                  : (RCTPromiseResolveBlock)resolve reject
                  : (RCTPromiseRejectBlock)reject)
{
  ...
  [[PHPhotoLibrary sharedPhotoLibrary]
      performChanges:^{
        PHFetchResult<PHAsset *> *fetched = [PHAsset fetchAssetsWithALAssetURLs:assets_ options:nil];
...
- (std::shared_ptr<facebook::react::TurboModule>)getTurboModule:
  ...
  return std::make_shared<facebook::react::NativeCameraRollManagerSpecJSI>(params);
}
