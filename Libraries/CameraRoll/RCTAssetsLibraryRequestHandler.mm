...
- (id)sendRequest:(NSURLRequest *)request withDelegate:(id<RCTURLRequestDelegate>)delegate {
  ...
  if (isPHUpload) {
    requestURL = [NSURL
        URLWithString:[@"ph" stringByAppendingString:[requestURL.absoluteString
                                                         substringFromIndex:RCTNetworkingPHUploadHackScheme.length]]];
  }
    ...
    [[PHImageManager defaultManager]
        requestImageDataForAsset:asset
                   ...
                   resultHandler:^(
                     ...
                     CFStringRef const mimeType =
                         UTTypeCopyPreferredTagWithClass(dataUTIStringRef, kUTTagClassMIMEType);

                     NSURLResponse *const response =
                         [[NSURLResponse alloc] initWithURL:request.URL
                                                   MIMEType:(__bridge NSString *)(mimeType)expectedContentLength:length
                                           textEncodingName:nil];
                     CFRelease(mimeType);
