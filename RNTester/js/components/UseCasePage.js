...
export default function UseCasePage(props: Props) {
	...
	const title = props.title ? (
	  <HeadingText text={props.title} />
	) : null;

	const description = props.description ?? "" ;
	const androidImage = props.android ? (
		<Image style={{height: 35, width: 30, margin: 2}} source={imagePaths.android} />
	) : null;
...
const styles = StyleSheet.create({
	titleView: {
		maxHeight: Math.round(ScreenHeight*0.10), 
	...
	iconContainer: {
		...
		position: 'absolute',
		marginTop: Math.round(ScreenHeight*0.10*0.65),
